...
async function setupReserves
    ...
    // setup mock reserves
    //////////////////////
    for (i=0; i < result.numAddedReserves; i++) {
        reserve = await MockReserve.new();

        let reserveType = MOCK_ID;
        if (numMock <= i && i < numMock + numFpr) {
            reserveType = FPR_ID;
        } else if (numMock + numFpr <= i && i < numMock + numFpr + numApr) {
            reserveType = APR_ID;
        }

        let reserveId = (genReserveID(reserveType, reserve.address)).toLowerCase();
        let rebateWallet;
